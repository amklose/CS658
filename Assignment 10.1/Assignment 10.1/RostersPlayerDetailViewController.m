...
{
    NSURL *url;
    if (!self.player.headshotUrl || [self.player.headshotUrl isEqualToString:@""]) {
        url = [NSURL URLWithString:DEFAULT_HEADSHOT_URL];
    } else {
        url = [NSURL URLWithString:self.player.headshotUrl];
    }
    UIImage *headshot = [UIImage imageWithData:[NSData dataWithContentsOfURL:url]];
    [self.headshotImageView setImage:headshot];
